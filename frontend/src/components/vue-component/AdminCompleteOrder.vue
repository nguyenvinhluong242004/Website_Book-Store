<template>
  <div class="admin-complete-order-body">
    ...
    <div class="admin-complete-order-box">
      <div class="admin-complete-order-box-title fs-4 fw-bold text-primary mb-4">
    ...
    <div
      class="modal d-block"
      id="modalAdminCompleteOrder"
      tabindex="-1"
      aria-labelledby="modalAdminCompleteOrderLabel"
      v-if="isModalVisible"
      @click.self="closeModal"
    >
      <!-- @click.self="closeModal" là khi nhấn vào vùng tối bên ngoài (là cái modalAdminCompleteOrder) thay vì nhấn vào modal là div con thì sẽ tắt -->
      <div
        ...
        <div class="modal-content p-0" style="width: 1000px; max-height: 90vh">
          <div class="modal-header">
            <h1
              ...
              id="modalAdminCompleteOrderLabel"
            >
              Thông tin chi tiết
            </h1>
            <button
...
<script>
import "../css-component/admin-complete-order.css";
    ...
    async handleRouteChange() {
      try {
        ...
        const response = await axiosInstance.get(
          `/admin/order/status-order?status=Completed&page=${this.currentPage}&per_page=${this.per_page}`
        );
    ...
    async goToPage(page) {
      if (page >= 1 && page <= this.total_page) {
        try {
          ...
          const response = await axiosInstance.get(
            `/admin/order/status-order?status=Completed&page=${page}&per_page=${this.per_page}`
          );
          if (response.status === 200) {
            this.orderList = response.data.orders;
            this.currentPage = response.data.page;
