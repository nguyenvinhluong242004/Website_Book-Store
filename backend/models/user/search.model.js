...
class SearchModel {
    ...
    static async findBookByKeyword(keyword, page = 1, perPage = 10) {
        ...
        const countResult = await pool.query(
            `SELECT COUNT(*) AS total FROM Book WHERE Book_Name ILIKE $1 OR Author ILIKE $1 OR Genre ILIKE $1`,
            [searchKeyword]
        );
        ...
        const result = await pool.query(
            `SELECT * FROM Book 
                 WHERE Book_Name ILIKE $1 OR Author ILIKE $1 OR Genre ILIKE $1 
                 LIMIT $2 OFFSET $3`,
    ...
    static async findBooksByFilters({
        ...
        if (keyword) {
            conditions.push('(Book_Name ILIKE $1 OR Author ILIKE $1 OR Genre ILIKE $1)');
